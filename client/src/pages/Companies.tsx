...
import { api, useGetCompaniesQuery, useCreateCompanyMutation, useUpdateCompanyMutation, useDeleteCompanyMutation } from '../services/api'
...
export default function Companies(){
  ...
  const [createIt] = useCreateCompanyMutation()
  const [updateIt] = useUpdateCompanyMutation()
  const [deleteIt] = useDeleteCompanyMutation()
  ...
  async function onSubmit(v:FormValues){
    if (editingId) {
      ...
      const patch = d(api.util.updateQueryData('getCompanies', { page, limit, search }, (draft:any)=>{ draft.data.unshift({ id: tempId, ...v }) }))
  ...
  async function del(id:number){
    const patch = d(api.util.updateQueryData('getCompanies', { page, limit, search }, (draft:any)=>{
      draft.data = draft.data.filter((x:any)=>x.id!==id)
    }))
  ...
  function editRow(it:any){
    setEditingId(it.id)
    ;['name','domain','phone','city','country'].forEach((k)=>setValue(k as any, it[k]))
  }
        ...
        <Card><Card.Body>
          ...
          <Table striped hover size="sm" className="align-middle mt-2">
            <thead>
              <tr>
                <th>ID</th>
                <th>name</th><th>domain</th><th>phone</th><th>city</th><th>country</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {(items as any[]).map((it:any)=> (
                <tr key={it.id}>
                  <td>{it.id}</td>
                  <td>{it.name}</td><td>{it.domain}</td><td>{it.phone}</td><td>{it.city}</td><td>{it.country}</td>
                  <td className="table-actions">
                    <Button variant="outline-secondary" size="sm" onClick={()=>editRow(it)}>Edit</Button>
                    {can(userRole, 'delete', 'companies') && (
  <Button variant="danger" size="sm" onClick={() => del(it.id)}>Delete</Button>
      ...
      <Col md={12}>
        <Card><Card.Body>
          <Card.Title>{editingId ? 'Edit' : 'Add'} Company</Card.Title>
